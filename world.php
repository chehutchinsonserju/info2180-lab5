...
if ($_SERVER["REQUEST_METHOD"] == "GET") 
{
    ...
    {
        $sanitizedCountry = ucwords(trim(filter_var($_GET["country"], FILTER_SANITIZE_STRING)));
        $stmt = $conn->query("SELECT * FROM countries WHERE name LIKE '%$sanitizedCountry%' ");
   ...
   else if ( isset($_GET['country']) && !empty($_GET['country']) && isset($_GET['context']) && $_GET['context'] == "cities" )
   {
    ...
    countries ON cities.country_code = countries.code  WHERE countries.name LIKE '%$sanitizedCountry%' ");
    $results = $stmt->fetchAll(PDO::FETCH_ASSOC);?>
